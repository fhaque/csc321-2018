...
def check_param_gradient(model, param_name, input_batch, target_batch):
    ...
    for count in range(1000):
        ...
        if np.any(rel > 1e-4):
            print('The loss derivative has a relative error of {}, which is too large.'.format(rel))
            print('param name: ', param_name, exact)
            return False
...
def load_partially_trained_model():
    part_file = open(PARTIALLY_TRAINED_MODEL, 'rb')
    obj = pickle.load(part_file, encoding='latin1')
    part_file.close()
...
#                      'hid_bias', 'output_bias']:
    for param_name in list(reversed(['word_embedding_weights', 'embed_to_hid_weights', 'hid_to_output_weights',
                       'hid_bias', 'output_bias'])):
