...
class Model(object):
    ...
    def back_propagate(self, input_batch, activations, loss_derivative):
        ...
        ###########################   YOUR CODE HERE  ##############################
        output_bias_grad = np.dot(loss_derivative.T, np.ones(activations.output_layer.shape[0]))
        hid_to_output_weights_grad = np.dot(loss_derivative.T, activations.hidden_layer)
        
        # intermediate calculations
        hid_grad = np.dot(loss_derivative, self.params.hid_to_output_weights)
        
        hid_linear_out_grad = (hid_grad * activations.hidden_layer * (1 - activations.hidden_layer)).T# z, derivative of logistic function
        
        hid_bias_grad = np.dot(hid_linear_out_grad, np.ones(activations.hidden_layer.shape[0]))
        embed_to_hid_weights_grad = np.dot(hid_linear_out_grad, activations.embedding_layer)
